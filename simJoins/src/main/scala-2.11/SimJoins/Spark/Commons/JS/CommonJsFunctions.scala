package SimJoins.Spark.Commons.JS
...
object CommonJsFunctions {
  ...
  def calcTF(tokenizedDocs: RDD[(Long, Set[String])]): RDD[(String, (Double, Long))] = {
    ...
    tokenizedDocs.flatMap(d => d._2.map(t => (t, 1.0))).combineByKey(create, add, merge).sortBy(_._2).zipWithIndex().map(x => (x._1._1, (x._1._2, x._2)))
  }
