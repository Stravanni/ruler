...
thresholds.foreach { threshold =>
  ...
  val ppjoinMioS = Calendar.getInstance()
  val ppjoinMioRes = PPJOINImp.joinPP(profiles, threshold, separatorID)
  ppjoinMioRes.cache()
  val numPPJOINMIORes = ppjoinMioRes.count()
  val ppjoinMioE = Calendar.getInstance()

  log.info("SPARKER - Num ppjoinmio " + numPPJOINMIORes)
  log.info("SPARKER - ERJOIN time " + msToMin(ppjoinMioE.getTimeInMillis - ppjoinMioS.getTimeInMillis))
  val pcPqMIO = ER.estimantePCPQ(newGT, ppjoinMioRes)
  log.info("SPARKER - ERJOIN PC " + pcPqMIO._1)
  log.info("SPARKER - ERJOIN PQ " + pcPqMIO._2)
  ppjoinMioRes.unpersist()
}
