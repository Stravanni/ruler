...
import RulER.DataStructure.PrefixEntry
...
object CommonJsFunctions {
  ...
  /**
    ...
    * (docId, posizione del token nel prefiss, lunghezza del documento)
    **/
  def buildPrefixIndex2(tokenizedDocOrd: RDD[(Long, Array[Long])], threshold: Double): RDD[(Long, Array[PrefixEntry])] = {
    val indices = tokenizedDocOrd.flatMap { case (docId, tokens) =>
      val prefix = JsFilters.getPrefix(tokens, threshold)
      prefix.zipWithIndex.map { case (token, pos) =>
        (token, PrefixEntry(docId, pos + 1, tokens.length))
      }
    }
    indices.groupByKey().filter(_._2.size > 1).map(x => (x._1, x._2.toArray.sortBy(_.docLen)))
  }
